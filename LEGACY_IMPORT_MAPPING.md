...
Use `--mode merge` to replace **named clients only** in `erp_database.sqlite` without truncating the whole database. Other clients, and (by default) local users, are kept. Merge does **not** write `.legacy_import_once.marker`. Startup auto-import is opt-in via `LEGACY_IMPORT_ON_STARTUP=1` and is skipped when the live database already has operational data.
