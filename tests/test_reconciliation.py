...
def test_reconciliation_accepts_tds_reduced_net_payable(client: TestClient):
    token = login(client, "admin", "Admin@1234")
    ...
    invoice = next(i for i in invs.json() if i["id"] == "INV-001")
    assert invoice["tds"] == pytest.approx(10.0)
    assert invoice["netPayable"] == pytest.approx(9990.0)

    report = _get_report(client, token)
    invoice_math = _section(report, "invoice_math")
    assert invoice_math["errors"] == 0
    assert all(issue["entity_id"] != "INV-001" for issue in invoice_math["issues"])
