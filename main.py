...
def _target_db_has_business_data() -> bool:
    """True when the live DB already has clients or invoices (not a blank first boot)."""
    db = SessionLocal()
    try:
...
def _maybe_run_legacy_import():
    """Optionally import a legacy snapshot on boot.

    This is destructive (replace mode truncates the target DB). It must be an
    explicit opt-in via LEGACY_IMPORT_ON_STARTUP, and it must never run against
    a database that already has clients or invoices.
    """
    if not _env_flag_enabled("LEGACY_IMPORT_ON_STARTUP"):
        return
    ...
    if _target_db_has_business_data():
        log.warning(
            "skipping startup legacy import because the target database already has data "
            "(place a marker or run migrate_sqlite.py explicitly to import)"
        )
        return
    try:
