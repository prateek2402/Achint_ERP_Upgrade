...
def _auto_apply_po_advance(client_id: int, db: Session, po_no: Optional[str] = None, invoice_no: Optional[str] = None):
    ...
    for po in po_rows:
        ...
        all_invoices.sort(key=lambda inv: invoice_ledger_sort_key(inv.inv_date, inv.invoice_no))
        max_allowed_by_invoice: dict[str, float] = {}
        for inv in all_invoices:
            base_amt = invoice_amount_for_po_base(inv, po.ret_base or "total")
            max_allowed_by_invoice[inv.invoice_no] = max(0.0, base_amt * (adv_pct / 100.0))

        existing_applied_rows = db.query(PaymentAllocation).join(
            PaymentHistory, PaymentAllocation.payment_id == PaymentHistory.id
        ).filter(
            PaymentHistory.client_id == client_id,
            PaymentAllocation.alloc_type == "po_advance_applied",
        ...
        for al in existing_applied_rows:
            inv_id = str(al.target_inv_id or "").strip()
            amount = float(al.amount or 0.0)
            remaining_allowed = max(0.0, max_allowed_by_invoice.get(inv_id, 0.0) - applied_by_invoice[inv_id])
            if amount <= 0 or remaining_allowed <= 0:
                db.delete(al)
                continue
            if amount > remaining_allowed:
                al.amount = remaining_allowed
                amount = remaining_allowed
            applied_by_invoice[inv_id] += amount
        db.flush()
        ...
        for inv in invoices:
            max_allowed = max_allowed_by_invoice.get(inv.invoice_no, 0.0)
