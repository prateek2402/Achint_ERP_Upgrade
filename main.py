...
def _maybe_run_legacy_import():
    ...
    try:
        existing_rows = (
            ...
            + db.query(PaymentHistory).count()
            + db.query(SystemSettings).count()
        )
    finally:
        db.close()
    if existing_rows:
        log.warning(
            ...
            "run migrate_sqlite.py manually if a merge or forced replace is intended"
        )
        return
    try:
...
def delete_purchase_order(po_no: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if po:
        ...
        if linked_invoice_count:
            raise HTTPException(
                status_code=400,
                detail="Cannot delete a purchase order that has linked invoices.",
            )
        db.delete(po)
        db.commit()
...
def recalculate_client_ledger(client_id: int, db: Session, preserve_manual_paid: bool = False, commit: bool = True):
    """
