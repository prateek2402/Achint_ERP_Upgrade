...
def _target_db_has_business_data() -> bool:
    """True when the live SQLite already has clients, POs, or invoices."""
    db = SessionLocal()
    try:
        if db.query(Client.id).first() is not None:
            return True
        if db.query(PurchaseOrder.id).first() is not None:
            return True
        if db.query(Invoice.id).first() is not None:
            return True
        return False
    finally:
        db.close()
...
def _maybe_run_legacy_import():
    """Optional one-time import of old_erp.sqlite.

    Must be explicitly enabled with LEGACY_IMPORT_ON_STARTUP. Presence of the
    legacy file alone must never truncate a populated live ERP database.
    """
    ...
    if not legacy_path.exists() or marker.exists():
        return
    try:
        if _target_db_has_business_data():
            log.warning(
                "Skipping startup legacy import: target database already has "
                "clients/POs/invoices. Run migrate_sqlite.py explicitly instead."
            )
            return
    except Exception as exc:
        log.exception("Could not inspect target database before legacy import: %s", exc)
        return
    try:
