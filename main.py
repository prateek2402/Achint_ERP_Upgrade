...
def _record_unallocated_payment_reversal(db: Session, row: UnallocatedPaymentRegister, amount: float) -> None:
    amount = round_inr_nearest(amount)
    if amount <= 0:
        return
    marker = f"UNALLOC_PAY_REG:{row.id}"
    existing = db.query(PaymentAllocation).filter(
        PaymentAllocation.alloc_type == "unallocated_reversal",
        PaymentAllocation.note_id == marker,
    ).first()
    if existing:
        return

    source_payment_id = (row.source_payment_id or "").strip()
    payment = db.query(PaymentHistory).filter(PaymentHistory.id == source_payment_id).first() if source_payment_id else None
    payment_id = payment.id if payment else f"UNALLOC-PAY-REV-{row.id}"
    if not payment:
...
def reverse_unallocated_payment_entry(entry_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    reversal_amount = float(row.balance or 0.0)
    _record_unallocated_payment_reversal(db, row, reversal_amount)
    row.status = "reversed"
    row.balance = 0.0
    db.commit()
